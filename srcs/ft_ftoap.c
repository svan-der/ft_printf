...
/*   Updated: 2019/12/10 16:30:36 by svan-der      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
size_t		handle_invalid(char **astr, t_opt *inval, t_ntoa *pref)
{
	...
	if (inval->nan)
	{
		if (pref->upper)
			...
			ft_memset(*astr, "nan", 3);
	}
	else
	{
		if (pref->upper)
			ft_memset(*astr + len, "INF", 3);
		if (!pref->upper)
			ft_memset(*astr + len, "inf", 3);
		(len) ? ft_memset(*astr, sign, 1) : 0;
	}
	return (size);
}
...
int		ft_dtoap(char **astr, t_dtoa *dtoa, t_spec *spec, t_ntoa *pref)
{
	...
	if (check_dbval(dtoa->fl_val, &dtoa->inval))
